...
PutStream.prototype._createWriter = function (helper, message, globalOptions){
  ...
  this._writer.onStats = function (stats){
    if (me._needAbort) return me._writer.abort (me._abortError);
    if (me._needClose) return me._writer.close ();
    me._ready = true;
    if (me._writer._size !== null){
      //setSize() was called but before the "stats" event, the request must
      //continue
      process.nextTick (function (){
        me._writer.continueRequest (me._size);
      });
    }

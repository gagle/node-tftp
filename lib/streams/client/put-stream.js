...
var PutStream = module.exports = function (remote, globalOptions, putOptions){
  ...
  stream.Writable.call (this, { highWaterMark: putOptions.highWaterMark });
  ...
  if (putOptions.size === 0){
    ...
    //the _write() function before. Then, when onStats() is called, send the
    //buffered chunk, but anyway when the file size is 0 it must be handled with
    //the next piece of code, so the implemented approach is the best.
...
PutStream.prototype.abort = function (message){
  if (this._writer) this._writer.abort (message);
};
...
var Server = module.exports = function (options, requestListener){
  ...
  if (!requestListener){
    requestListener = function (req, res){
      if (req.method === "GET"){
        fs.createReadStream (this.root + req.path)
        ...
        req.pipe (fs.createWriteStream (this._root + req.path)
            .on ("error", function (error){
              //Don't need to wait to the abort event
              this.emit ("error", error);
              req.abort ();
  ...
  this._closed = false;
  this._controller = new Controller (options, requestListener);

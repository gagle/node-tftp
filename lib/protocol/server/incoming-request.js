...
IncomingRequest.prototype.continueRequest = function (){
  //The request is paused until setSize() is called
  if (this._requestExtensions === null){
    this.onContinue ();
  }else{
    this._sendAndRetransmit (this._responseExtensions);
  }
};
...
IncomingRequest.prototype._sendOackMessage = function (extensions){
  ...
  ext = packets.oack.serialize (ext);
  if (this._isRRQ){
    //Prepare the extensions
    this._responseExtensions = ext;
  }else{
    this._sendAndRetransmit (ext);
  }
};
...
Window.prototype.eof = function (){
  return this._end;
};

Window.prototype.feed = function (block, cb){
  var me = this;
  this._writer._request.sendBlock (this._block++, block, function (){
    me._sent++;
    me._end = block.length < me._writer._blockSize;
    
    if (me._sent === me._writer._blockSize || me._end){
      //Wait for the ack
      me._cb = cb;
    }else{
      cb ();
...
Window.prototype.lastBlockSent = function (){
...
Writer.prototype.send = function (buffer, cb){
  ...
  (function newBlock (){
    ...
    //but this is only a security feature. The end of the file occurs when the
    //block has a size smaller than blockSize
    me._window.feed (me._blockMaker.next (), onProcessed);
  })();
};
...
Writer.prototype._onAck = function (ack){
  ...
  if (ack.block === this._window.lastBlockSent ()){
    this._window.resume ();
  }else{
    console.log("out")
  }

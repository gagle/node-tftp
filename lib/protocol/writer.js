...
var Writer = module.exports = function (remote, globalOptions, putOptions){
  ...
  this._rollover = globalOptions.extensions.rollover;
...
Window.prototype.feed = function (block, cb){
  ...
  this._writer._request.sendBlock (this._block, block, function (){
    ...
    //Rollover
    if (me._block === 65536){
      me._block = me._writer._rollover;
    }
    
    if (me._sent === me._writer._windowSize || me._end){
      //Wait for the ack
      me._cb = cb;
      me._sent = 0;
    }else{
...
Writer.prototype._onAck = function (ack){
  ...
  //else, invalid ack, simply omit it in order to avoid duplicates
};
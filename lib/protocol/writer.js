...
var Writer = module.exports = function (Request, args){
  Request.call (this, args);
  
  this._blockMaker = null;
  this._window = null;
  this._size = args.opOptions ? args.opOptions.size : args.size;
};
...
Writer.prototype.send = function (buffer, cb){
  ...
  //TODO, guardar next y newBlock dentro de writer en el prototipo
  var next = function (){
    if (me._window.isEOF ()){
      me._onClose = cb;
      me._close ();
...
Writer.prototype._onError = function (error){
  if (this._window) this._window._retransmitter.reset ();
  this.onError (error);
};
...
Writer.prototype._onReady = function (stats){
  this._blockMaker = new BlockMaker (stats.blockSize, this._size);
  this._window = new Window (stats.blockSize, stats.windowSize, stats.rollover,
      this);
...
var Window = function (blockSize, windowSize, rollover, request){
  ...
  var me = this;
  this._retransmitter = request._createRetransmitter ();
  this._restransmitterSendFn = function (){
    me._blocks.forEach (me._sendFn);
  };
};

//TODO, eliminar y poner .isEOF publico
...
Window.prototype.feed = function (block, cb){
  ...
  if (!--this._pending || this._eof){
    ...
    //Start the timer
    this._retransmitter.start (this._restransmitterSendFn);

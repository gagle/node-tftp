...
Reader.prototype._retransmit = function (){
  ...
  this._retransmitter.start (function (){
    var block = me._blockToRetransmit ();console.log(block)
...
Reader.prototype._sortWindow = function (){
  if (this._mayRollover){
    //Example: [65535, 65534, 1, 0] -> [65534, 65535, 0, 1]
...
Reader.prototype._onData = function (message){//console.log("<< " + message.block)
  ...
  //Check is the curernt window may rollover
  var windowEnd = this._windowStart + this._windowSize - 1;
  if (!this._mayRollover){
    this._mayRollover = windowEnd - 65535 > 0;
  }
  
  //Check the validity of the block if the current window doesn't rollover and
  //the block is outside the valid range, it could be a duplicate (sorcerer's
  //apprendice syndrome)
  if (!this._mayRollover &&

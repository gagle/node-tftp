...
var Reader = module.exports = function (remote, options){
	...
	.on ("close", function (){
		...
		if (me._error){
			...
			//TODO comprobar si la descarga se ha completado o si el servidor se ha
	...
	this._sendMessage (packets.rrq.serialize (remote, this._options), function (){
		me._state = RRQ_SENT;
	});
};
...
Reader.prototype._onError = function (message){
	...
	if (this._state === RRQ_SENT && message.code === 8){
		...
		this._error = new TFTPClientError ("Server: " + message.message);
		this._close ();
	}
};

Reader.prototype._onMessage = function (buffer){
	//RRQ or ACK packet sent, valid packets: DATA, ERROR
...
Reader.prototype._sendErrorAndClose = function (code, message){
	...
	this._send (packets.error.serialize (code, message), function (){
		me._error = new TFTPClientError ("Client: " + (errors[code] || message));
		me._close ();
	});
};
...
Reader.prototype._sendMessage = function (buffer, cb){
	...
	this._timer = setTimeout (function (){
		if (!me._retries){
			me._error = new TFTPClientError ("Timeout reached");
			me._close ();
		}else{
			me._retries--;
...
Reader.prototype._close = function (){
	...
	//The reader can emit an error when the socket is closed. If it's the first
	//case, the error is not relevant because the reader is aborted just before.
	//If it's the second case, the abort it's useless because the reader is
	//already closed
	this._closing = true;
	this._socket.close ();
};

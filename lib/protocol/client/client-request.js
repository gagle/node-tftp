...
ClientRequest.prototype.abort = function (message){
...
ClientRequest.prototype._close = function (error){
...
ClientRequest.prototype._sendAck = function (block){//console.log(">> " + block)
  this._state = ACK_SENT;
  this._send (packets.ack.serialize (block));
};

ClientRequest.prototype._sendBlock = function (block, buffer){//console.log(">> " + block)
  this._state = BLK_SENT;
  this._send (packets.data.serialize (block, buffer));
};

ClientRequest.prototype._sendMaxSizeErrorAndClose = function (){
  this._send (packets.error.serialize (errors.EDENY));
  this._close (new Error (errors.EFBIG));
};

ClientRequest.prototype._sendErrorAndClose = function (code){
  var message;
...
ClientRequest.prototype._closeWithError = function (code){
  var message;
  if (typeof code === "number"){
    message = errors.rfc[code];
  }else{
...
ClientRequest.prototype._sendAndRetransmit = function (buffer){
  this._send (buffer);
  var me = this;
...
ClientRequest.prototype._send = function (buffer){
...
ClientRequest.prototype._createRetransmitter = function (){

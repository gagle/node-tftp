...
Request.prototype._open = function (){
  ...
  this._socket = dgram.createSocket ("udp4", function (message, rinfo){
    ...
    //If the server send invalid packets then the client may not work properly
    me._resetTimeout ();
...
Request.prototype._resetTimeout = function (){
  ...
  this._blockTimers.forEach (function (timer){
    clearTimeout (timer);
  });
  this._blockTimers = [];
};

Request.prototype._checkSize = function (infinite){
...
Request.prototype._onOack = function (message){
  ...
  //TODO: instead of failing, return a hash with the extensions (custom extensions)
  //if the client doesn't set any custom extension, then fail
  for (var key in message){
    if (this._globalOptions.extensions[key] === undefined){
  ...
  if (this._checkSize (infinite)) return;
...
Request.prototype.sendAck = function (block, cb){//console.log(">> " + block)
  var me = this;
...
Request.prototype.sendBlock = function (block, buffer, cb){//console.log(">> " + block)
  var me = this;
  buffer.__block = true;
  this.sendMessage (packets.data.serialize (block, buffer), function (){
    me._state = BLK_SENT;
    cb ();
  });
};
...
Request.prototype.sendMessage = function (buffer, cb){
  ...
  var id = setTimeout (function (){
    if (buffer.__retries === me._globalOptions.retries){
      me.close (new Error ("Timed out"));
    }else{
      if (!buffer.__retries) buffer.__retries = 0;
      buffer.__retries++;
      me.sendMessage (buffer);
    }
  }, this._timeout);
  
  if (buffer.__block){
    //Store the timers of each block in a separate array
    this._blockTimers.push (id);
  }else{
    this._timer = id;
  }*/

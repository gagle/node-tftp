...
Request.prototype._open = function (){
  ...
  this._socket = dgram.createSocket ("udp4", function (message, rinfo){
    //if (message.readUInt16BE (2) === 6) return;
...
Request.prototype._onOack = function (message){
  ...
  if (this._isRRQ){
    ...
    var me = this;
    this.sendAck (0, function (){
      me.emit ("ready", stats, true);
    });
  }else{
    this.emit ("ready", stats);
  }
...
Request.prototype.sendAck = function (block, cb){//console.log(">> " + block)
  var me = this;
  this._send (packets.ack.serialize (block), function (){
    me._state = ACK_SENT;
    if (cb) cb ();
  });
};
...
Request.prototype.sendBlock = function (block, buffer, cb){//console.log(">> " + block)
  var me = this;
  //buffer.__block = true;
  this._send (packets.data.serialize (block, buffer), function (){
    me._state = BLK_SENT;
    cb ();
  });
};

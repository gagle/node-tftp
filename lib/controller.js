...
var Controller = module.exports = function (server, options, listener){
  this._server = server;
  this._options = options;
  this._listener = listener || server.requestListener;
...
Controller.prototype.request = function (message, address, port){
  ...
  if (this._d){
    this._d = false
    return this._sendErrorAndClose (socket, errors.EDENY);
  }
  ...
  if (op === opcodes.RRQ){
    ...
    var ps = new PutStream (socket, message, this._options, function (file){
      //Call the request listener
      var gs = new GetStream (ps);
      gs.method = "GET";
      gs.file = file;
      //The stats event is emitted on the get stream
      ps._gs = gs;
      me._listener.call (me._server, gs, ps);
    });
  }else if (op === opcodes.WRQ){
    if (this._options.denyPUT){
      return this._sendErrorAndClose (socket, errors.ENOPUT);
    }
    var gs = new GetStream (socket, message, this._options, function (file){
      //Call the request listener
      gs.method = "PUT";
      gs.file = file;
      me._listener.call (me._server, gs, new PutStream ());
    });
  }else{
